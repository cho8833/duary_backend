...
func Test_GenerateOccurrence_of_daily_cross_day_by_one_day(t *testing.T) {
	t.Logf("Daily Recur : 자정을 넘어가는 일정도 잘 생성하는지 확인")
	rangeStartDate := time.Date(2025, 5, 1, 0, 0, 0, 0, time.UTC)
	rangeEndDate := rangeStartDate.AddDate(0, 0, 1)
	recurStartDate := time.Date(2025, 4, 30, 0, 0, 0, 0, time.UTC)
	recurEndDate := recurStartDate.AddDate(0, 0, 7)
	testTime := time.Date(2025, 5, 1, 23, 0, 0, 0, time.UTC)
	...
	expected := time.Date(2025, 5, 1, 23, 0, 0, 0, time.UTC)
	if !result[0].StartDateTime.Equal(expected) {
		t.Fatalf("Wrong Occurence %s", shared.ToString(result[0]))
	}
}
...
func Test_GenerateOccurrence_of_daily_cross_day_by_one_month(t *testing.T) {
	recurStartDate := time.Date(2025, 5, 30, 0, 0, 0, 0, time.UTC)
	recurEndDate := time.Date(2025, 6, 10, 0, 0, 0, 0, time.UTC)
	testTime := time.Date(2025, 5, 30, 23, 0, 0, 0, time.UTC)
